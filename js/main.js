(() => {
  ...
  if (quickLinks && quickToggle) {
    const mobileToolbar = matchMedia("(max-width: 620px)");
    const setExpanded = (open) => {
      document.body.classList.toggle("toolbar-expanded", open);
      quickToggle.setAttribute("aria-expanded", String(open));
      quickToggle.setAttribute("aria-label", open ? "關閉快速工具列" : "開啟快速工具列");
      if (!open && quickLinks.contains(document.activeElement)) quickToggle.focus({ preventScroll: true });
    };
    const syncToolbar = () => {
      const condensed = mobileToolbar.matches && scrollY > 160;
      document.body.classList.toggle("toolbar-condensed", condensed);
      if (!condensed) {
        if (document.activeElement === quickToggle) quickLinks.querySelector("a,button")?.focus({ preventScroll: true });
        setExpanded(false);
      } else if (!document.body.classList.contains("toolbar-expanded") && quickLinks.contains(document.activeElement)) {
        quickToggle.focus({ preventScroll: true });
      }
    };
    quickToggle.addEventListener("click", () => setExpanded(!document.body.classList.contains("toolbar-expanded")));
    document.addEventListener("pointerdown", (event) => {
      if (document.body.classList.contains("toolbar-expanded") && !quickLinks.contains(event.target) && !quickToggle.contains(event.target)) setExpanded(false);
    });
    document.addEventListener("keydown", (event) => {
      if (event.key === "Escape" && document.body.classList.contains("toolbar-expanded")) setExpanded(false);
    });
    addEventListener("scroll", syncToolbar, { passive: true });
    addEventListener("resize", syncToolbar);
    syncToolbar();
  }
